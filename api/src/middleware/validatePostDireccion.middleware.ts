...
const createDireccionSchema = yup.object({
  params: yup.object({
    personadni: yup.string().required().min(7).max(8).matches(regexOnlyNumber, 'params.personadni must be a number')
  }),
  body: yup.object({
    calle:  yup.string().required().min(2),
    altura: yup.string().required().min(1).matches(regexOnlyNumber, 'body.altura must be a number'),
